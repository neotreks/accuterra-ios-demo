...
extension Int64 {
    func humanFileSize() -> String {
        let estimateSizeInGb = Double(self) / 1000.0 / 1000.0 / 1000.0
        if estimateSizeInGb >= 1 { //GB
            return String(format: "%.1f GB", estimateSizeInGb)
        } else if estimateSizeInGb >= 0.001 { //MB
            return String(format: "%.1f MB", estimateSizeInGb * 1000)
        } else { //KB
            return String(format: "%.1f KB", estimateSizeInGb * 1000 * 1000)
        }
    }
}
