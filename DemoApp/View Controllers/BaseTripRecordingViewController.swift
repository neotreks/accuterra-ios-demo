...
class BaseTripRecordingViewController : BaseDrivingViewController {
    ...
    var tripRecorder: ITripRecorder {
        if _tripRecorder == nil {
            ...
            tripUuid = try! _tripRecorder.getActiveTrip()?.tripInfo.uuid
        }
    ...
    deinit {
        ...
        if let currentStatus = (try? tripRecorder.getActiveTrip()?.recordingInfo.status),
    ...
    func updateRecordingButtons() {
        let activeTrip = try? tripRecorder.getActiveTrip()
    ...
    func updateStatistics(distance: Double?, drivingTime: Int64?) {
    ...
    @IBAction func onRecordingStartClicked(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: { (action) in
            ...
            let activeTrip = try? self.tripRecorder.getActiveTrip()
    ...
    @IBAction func onRecordingFinishClicked(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: { (action) in
            do {
                guard let uuid = try self.tripRecorder.getActiveTrip()?.tripInfo.uuid else {
    ...
    func loadAndShowPoiInfo(tripUuid: String, poiId: String) {
        ...
        do {
            guard try service.getTripByUUID(uuid: tripUuid) != nil else {
                throw "Trip #\(tripUuid) not found.".toError()
            }
            guard let poi = try service.getTripPoiByUuid(poiUuid: poiId) else {
